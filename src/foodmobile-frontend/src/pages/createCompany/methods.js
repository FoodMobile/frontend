...
async function submitCreateCompany(thisVar) {
    ...
    try {
        ...
        let payloadJoinCompany = new URLSearchParams();
        payloadJoinCompany.append("companyGuid",res.data.data.guid)
        payloadJoinCompany.append("userGuid",userGUID)

        
        const res2 =  await axios.post(`${thisVar.context.ip}${thisVar.context.endpoints.joinCompany}`, payloadJoinCompany)
